...
const CardTwo = ({tempProducts}) => {
  return (
    <>
       {
            tempProducts && tempProducts.map((pro, indx) => {

              return (
                
                <div className='col-10 col-sm-6 col-md-4 col-lg-3 p-3 m-auto p-0 '>
                  <Link to={`/product/${pro.id}`} className='link-dark text-decoration-none' >
                    <div className="card h-100 position-relative border-0 shadow "  key={indx}>
                      <img src={pro?.images[0]} className="card-img-top" alt={pro?.title} height='220px' />
                      <div className="position-absolute top-0 start-0 fs-5 headingThreeCss px-2 mt-2 text-light">
                        {pro.category}
                      </div>
